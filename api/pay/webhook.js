import Stripe from "stripe";
import { statusCartFunction, statusOrderFunction } from '../controllers/OrdersController.js';

const { ENDPOINT_SECRET, SIGNING_SECRET } = process.env; 

const endpointSecret = /* "whsec_0b5f910e80d53f35ab1415e1b1e44fddd72476ce111c907e4ef053a8f8214f1a"; */ENDPOINT_SECRET
const stripe = Stripe(/* 'sk_test_51LSoUXFrlpRCY5YH7F7s7KDDAOsF4LAeXJyAJrHjUUSObyUbcDECpGu7N2Afj6N9P1aa7hdc1Ca85x4fSDUJebER00IklWuRZ3' */SIGNING_SECRET);

export const PaymentResponse = async (req, res)=>{
  const sig = req.headers['stripe-signature'];
  const id = req.body.client_reference_id;
  console.log({CARRITO_ID: id});
  let data;
  let eventType;
  try {
    // await statusCartFunction(id);

    const payload = req.body;
    const payloadString = JSON.stringify(payload, null, 2);
    const header = stripe.webhooks.generateTestHeaderString({
      payload: payloadString,
      secret: endpointSecret 
    });
    let event = stripe.webhooks.constructEvent(payloadString, header, endpointSecret);
    // console.log(`Webhook Verified: `, event);
    data = event.data.object;
    eventType = event.type;
    console.log(eventType);

    if (eventType === 'payment_intent.payment_failed') {
      console.log(data.status);
      // await statusOrderFunction(id, 'cancel');
    } else if (eventType === 'payment_intent.succeeded') {
      console.log(data.status);
      // await statusOrderFunction(id, 'paid');
    };
    res.send('pay')
  } catch (err) {
    console.log(`Webhook Error: ${err.message}`)
    res.status(400).send(`Webhook Error: ${err.message}`);
    return;
  };
};